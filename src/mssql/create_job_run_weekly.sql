...
USE msdb ;  
GO  
CREATE CREDENTIAL CatalogApplicationCredential WITH IDENTITY = 'DESKTOP-K00VI09\Huy',   
    SECRET = '12345';  
GO  
...
EXEC  msdb.dbo.sp_add_job @job_name=N'Rundemo', 
		...
		@owner_login_name=N'DESKTOP-K00VI09\Huy', 
		@notify_email_operator_name=N'OperatorTest', @job_id = @jobId OUTPUT
select @jobId
GO
EXEC msdb.dbo.sp_add_jobserver @job_name=N'Rundemo', @server_name = N'DESKTOP-K00VI09'
GO
USE [msdb]
GO
EXEC msdb.dbo.sp_add_jobstep @job_name=N'Rundemo', @step_name=N'RunMain', 
		...
		@command=N'/ISSERVER "\"\SSISDB\FA_Project02\SSIS_ETL_StagingData\AdsBI.dtsx\"" /SERVER "\"DESKTOP-K00VI09\"" /Par "\"$ServerOption::LOGGING_LEVEL(Int16)\"";1 /Par "\"$ServerOption::SYNCHRONIZED(Boolean)\"";True /CALLERINFO SQLAGENT /REPORTING E', 
